...
class MaskingActivationLayer(tf.keras.layers.Layer):
    ...
    @tf.function
    def get_masks_for_batch(self, inputs):
        '''
        Inputs:
        - chosen_types:         (BATCH_SIZE, SEQ_LEN-1, 1)
        - song_tokens:          (BATCH_SIZE, SEQ_LEN-1, 11)
        - seq_scores:           (BATCH_SIZE, SEQ_LEN-1, 1391)
        '''
        # Collect inputs
        chosen_types, song_tokens, seq_scores = inputs
        # Define mask (output) using a TensorArray
        batch_size = tf.shape(chosen_types)[0]
        mask = tf.TensorArray(dtype=tf.bool, size=batch_size)

        # Define masking function
        # @tf.function
        def get_mask_for_all_tokens(inputs):
            chosen_type, scores, idx, batch_idx = inputs   # Type is a scalar, scores is a 1391 tensor, seq_el and batch_el are scalars
            # Cast type and song tokens to int
            chosen_type = tf.cast(chosen_type, dtype=tf.int32)
            song = tf.cast(song_tokens[batch_idx], tf.int32) # Get the current song using batch index

            ## SETUP ##

            # Define the default variables and flags
            until_idx = idx + 1
            default_token_parts   = tf.repeat(tf.constant(True, dtype=tf.bool), len(conf.INPUT_RANGES)-1)
            default_flag          = False
            ...
            allowed_instruments   = tf.repeat(tf.constant(0, dtype=tf.int32), conf.INPUT_RANGES["instrument"])
            ...
            forbidden_instruments = tf.repeat(tf.constant(0, dtype=tf.int32), conf.INPUT_RANGES["instrument"])
            ...
            if chosen_type == 0 or chosen_type == 2 or chosen_type == 7:
                ...
                if tf.size(tf.where(song[:until_idx, 0] == 1)[:,0]) == 0:
                    ...
                    default_token_parts = tf.constant([True, True, True, True, True, False, True, True, True, True])
                    default_flag = True
                else:
            ...
            if default_flag:                
                ...
                # Unsupported branch, but needed by autograph. 
                tf.Assert(False, ["Unsupported type", chosen_type])
                return tf.cast(
                    tf.zeros(conf.input_ranges_sum - conf.INPUT_RANGES['type']),
                    dtype=tf.bool)

        # Iterate over batches and execute the above function in parallel
        for batch_el in range(batch_size):
            batch_mask = tf.map_fn(
                fn=get_mask_for_all_tokens, elems=(
                    chosen_types[batch_el], 
                    seq_scores[batch_el],
                    tf.range(conf.SEQ_LEN-1),
                    tf.repeat(batch_el, conf.SEQ_LEN-1)     # An index of the current batch element
                ), fn_output_signature=tf.TensorSpec(       # Total: a SEQ_LEN-1 * 1391 tensor
                (conf.input_ranges_sum - conf.INPUT_RANGES['type']),
                dtype=tf.bool
            ), parallel_iterations=2048)
            # TODO: why doesn't the following code work?
            # batch_mask = tf.vectorized_map(
            #     fn=get_mask_for_all_tokens, elems=(
            #         chosen_types[batch_el], 
            #         seq_scores[batch_el],
            #         tf.range(conf.SEQ_LEN-1),
            ...
            #     )
            # )
            mask = mask.write(batch_el, batch_mask)
        return mask.stack()
    ...
    def call(self, inputs, training=True):
        ...
        passes through vectorized_map --> get_mask_fro_all_tokens to debatch
        '''
        ...
        masks = self.get_masks_for_batch([
